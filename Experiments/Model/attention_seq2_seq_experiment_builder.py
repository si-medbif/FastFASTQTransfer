...
def build_attention_seq2seq_model (configuration: Configuration, model_full_path: str, training_hist_path: str, encoder_input_data, decoder_input_data, decoder_target_data) -> Model :
    ...
    encoder = LSTM(configuration.latent_dim, return_sequences=True, return_state=True)
...
def transform_model (full_model, configuration: Configuration) -> (Model,Model) :
    ...
    # Result from Encoder
    encoder_output = Input(shape=(configuration.latent_dim,), name='encoder_output')

    # H and C State from Encoder
    decoder_state_input_h = Input(shape=(configuration.latent_dim,), name='decoder_h')
    decoder_state_input_c = Input(shape=(configuration.latent_dim,), name='decoder_c')
...
def predict_attention_seq2seq_qscore_set (encoder_input_data : Any, raw_score_data: Any, encoder_model : Model, decoder_model: Model, configuration: Configuration, mse_log_full_path: str, array_diff_full_path:str) -> list:
    ...
    for data_index in range(0, len(encoder_input_data)) :
        ...
        # Extract encoder_output, h_state, c_state
        encoder_output = encoder_result[0]
        state_h = encoder_result[1]
        state_c = encoder_result[2]
...
def main(args) :
    ...
    # Predict data
    pred = predict_attention_seq2seq_qscore_set(encoder_input_data, raw_score_data, encoder_model, decoder_model, configuration, mse_log_full_file_name, array_diff_full_file_name)
